...
kcart.prototype.updateSession = function (items) {
    ...
    console.log('updateSession - ITEMS', items);
...
kcart.prototype.upgradeLineItems = function () {
    ...
    for (var i = 0; i < nodes.length; i++) {
        ...
        if (item.minusBtn) {
            ...
            item.minusBtn.onclick = function () {
                kcart.minusItem(this.lineItem.productId)
            };
        }
        //upgrade plus button
        ...
        if (item.plusBtn) {
            ...
            item.plusBtn.onclick = function () {
                kcart.plusItem(this.lineItem.productId)
            };
        }
        ...
        if (item.removeBtn) {
            ...
            item.removeBtn.onclick = function () {
                kcart.removeItem(this.lineItem.productId)
            };
        }
...
kcart.prototype.calculateTotals = function () {
    ...
    //set the amount for PAAY 3DS if it exists - the amount must be an integer, so rounding up to the nearest whole number
    var paayAmount = document.getElementById('paayAmount');
    if (paayAmount)
        paayAmount.value = Math.ceil(this.grandTotal);

    return {
        subTotal: this.subTotal,
        shipTotal: this.shipTotal,
